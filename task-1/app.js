...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        wrap: cardNode.className , // здесь элемент с классом .product
...
function hideMoreText(card) {
    ...
    card.productName.childNodes[0].outerHTML = ''
...
function showMoreText(card) {
    ...
    let someText = 'Я произвольный текст. ';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterbegin', `<div class = desc >${someText}</div>`);
